  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:980}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:13}}>
      <div style={{background:c.bg1,border:`1px solid ${c.line}`,borderRadius:13,padding:14}}>
        ...
        {crewCount===0?<div style={{fontFamily:UI,fontSize:12.5,color:c.t2}}>None yet. Feed Claude the call sheet.</div>:DEPTS.map(d=>(crew[d.k]||[]).length?<div key={d.k} style={{marginBottom:10}}><div style={{fontFamily:MONO,fontSize:10,color:c.t2,marginBottom:5}}>{d.label}</div>{(crew[d.k]||[]).map(m=><div key={m.id||m.name} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:6}}><div style={{minWidth:0}}><div style={{fontFamily:UI,fontSize:13,color:c.t0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{m.name}</div>{m.role&&<div style={{fontFamily:UI,fontSize:11,color:c.t2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{m.role}</div>}</div><div style={{display:"flex",gap:9,flexShrink:0}}>{m.phone&&<a href={waHref(m.phone)} target="_blank" rel="noopener noreferrer" style={{color:c.t1}} title={`WhatsApp ${m.phone}`}><Phone size={14}/></a>}{m.email&&<a href={`mailto:${m.email}`} style={{color:c.t1}} title={m.email}><Mail size={14}/></a>}</div></div>)}</div>:null)}
      </div>
      <div style={{background:c.bg1,border:`1px solid ${c.line}`,borderRadius:13,padding:14}}>
