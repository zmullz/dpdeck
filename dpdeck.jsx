...
function Dashboard({project,onOpen,onNav}){
  ...
  // Key Contacts widget: the director + producers (the first entries), plus the Workflow Supervisor (dailies & lab contact) so they're always one tap away.
  const keyContacts=(()=>{const out=(project.contacts||[]).slice(0,5).slice();const ws=(project.contacts||[]).find(ct=>/workflow supervisor/i.test(ct.role||""));if(ws&&!out.includes(ws))out.push(ws);return out;})();
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:980}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:13}}>
      ...
      <div style={{background:c.bg1,border:`1px solid ${c.line}`,borderRadius:13,padding:14}}>
        ...
        {keyContacts.length===0?<div style={{fontFamily:UI,fontSize:12.5,color:c.t2}}>None yet.</div>:keyContacts.map(ct=><div key={ct.id||ct.name} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:7}}><div style={{minWidth:0}}><div style={{fontFamily:UI,fontSize:13,color:c.t0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ct.name}</div>{ct.role&&<div style={{fontFamily:UI,fontSize:11,color:c.t2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ct.role}</div>}</div><div style={{display:"flex",gap:9,flexShrink:0}}>{ct.phone&&<a href={waHref(ct.phone)} target="_blank" rel="noopener noreferrer" style={{color:c.t1}} title={`WhatsApp ${ct.phone}`}><Phone size={14}/></a>}{ct.email&&<a href={`mailto:${ct.email}`} style={{color:c.t1}}><Mail size={14}/></a>}</div></div>)}
      </div>
      <div style={{background:c.bg1,border:`1px solid ${c.line}`,borderRadius:13,padding:14}}>
